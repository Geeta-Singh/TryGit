...
public class DBhandler{
	...
	ArrayList<FriendModel> repo=new ArrayList<FriendModel>(); 
	...
	try
	{
		...
		System.out.println("connection");
		Connection con= db.createConnection();
		PreparedStatement ps= (PreparedStatement) con.prepareStatement("SELECT * FROM NEWSADD");
		ResultSet rs= ps.executeQuery();
		//System.out.println("rs is\n"+rs);
		while(rs.next())
		{
	...
	return (News_name);
		...
		public void dbFinancialInsert(FinancialInsertModel usr) throws FileNotFoundException{
			try {
				DBconnection db=new DBconnection ();	
				Connection con=db.createConnection();	
	...
	try
	{
		...
		Connection con= db.createConnection();
		PreparedStatement ps1= (PreparedStatement) con.prepareStatement("select s.INTEREST from STUDENT s where S_ID=?");
		ps1.setString(1,sid);
		...
		PreparedStatement ps= (PreparedStatement) con.prepareStatement("SELECT DESCRIPTION FROM ANNOUNCEMENT a where a.INTEREST IN (?)");
	...
	//	System.out.println(rs1.next());
		while(rs1.next())
		{
			...
			System.out.println("annuncement heading="+rs1.getString(1));
	...
	public void insertAnnouncement(ArrayList<AnnouncementModel> array) throws Exception
	{
	DBconnection db=new DBconnection();
	Connection con=(Connection) db.createConnection();
	java.util.Date date= new java.util.Date();
	...
	{	
		...
		try 
		{
			...
			pstmt =c.prepareStatement("select FACULTY_NAME , EMAIL from FACULTY where FACULTY_ID='" + getFaculty.getFaculty_Id() + "';");  
			...
			ps =c.prepareStatement("select PASSWORD from CREDENTIALS where U_ID='" + getFaculty.getFaculty_Id() + "';");
			rs = ps.executeQuery();
	...
	{	
		...
		try 
		{
			...
			pstmt =c.prepareStatement("update FACULTY SET FACULTY_NAME = ?"+","+"EMAIL=?"+" where FACULTY_ID='" + updateFaculty.getFaculty_Id() + "';");  
			//System.out.println(getFaculty.getFaculty_Id());
			pstmt.setString(1, updateFaculty.getFaculty_Name());
			pstmt.setString(2, updateFaculty.getEmail());
			pstmt.executeUpdate();
		  
			ps =c.prepareStatement("update CREDENTIALS SET PASSWORD = ?"+" where U_ID='" + updateFaculty.getFaculty_Id() + "';"); 
	...
	{	
		...
		try 
		{
			...
			pstmt =c.prepareStatement("DELETE FROM FACULTY WHERE FACULTY_ID='" + deleteFaculty.getFaculty_Id() + "';");  
			//System.out.println(getFaculty.getFaculty_Id());
			pstmt.executeUpdate();
			ps =c.prepareStatement("DELETE FROM CREDENTIALS WHERE U_ID='" + deleteFaculty.getFaculty_Id() + "';");  
			//System.out.println(getFaculty.getFaculty_Id());
			ps.executeUpdate();
	...
	{	
		...
		try 
		{
			...
			pstmt =c.prepareStatement("select ADDRESS,DOB,F_NAME,L_NAME,EMAIL,PHONE,CURRENT_SEMESTER from STUDENT where S_ID=?");  
	...
	{	
		...
		try 
		{
			...
			pstmt =c.prepareStatement("update STUDENT SET ADDRESS = ?"+","+"F_NAME = ? " + ","  + "L_NAME = ?" + ","+"EMAIL = ?" + "," + "PHONE = ?" + "," + "CURRENT_SEMESTER = ? " + " where S_ID=?;");  
		...
		status =	pstmt.executeUpdate();
			...
			ps =c.prepareStatement("update CREDENTIALS SET PASSWORD = ? "+"where U_ID='" + updateStudent.getSid() + "';");
	...
	{	
		...
		try 
		{
			...
			pstmt =c.prepareStatement("DELETE FROM STUDENT WHERE S_ID='" + deleteStudent.getSid() + "';");  
			pstmt.executeUpdate();

			ps =c.prepareStatement("DELETE FROM CREDENTIALS WHERE U_ID='" + deleteStudent.getSid() + "';");
			ps.executeUpdate();
		}
		catch(Exception e)
		{
	...
	public  ArrayList<StudentModel> display(String RollNumber) throws ClassNotFoundException, SQLException {
		dbc = new DBconnection();
		Connection c = null;
		c = dbc.createConnection();
		...
		PreparedStatement ps = c.prepareStatement("select * from STUDENT where S_ID =?");
	...
	public  ArrayList<String> fetchallstudents() throws ClassNotFoundException, SQLException {
		dbc = new DBconnection();
		Connection c = null;
		c = dbc.createConnection();
		...
		PreparedStatement ps = c.prepareStatement("select S_ID, F_NAME, L_NAME from STUDENT");
		ResultSet rs =  (ResultSet)ps.executeQuery();
		while (rs.next()) 
		{
			String list;
			...
			rs.getString("F_NAME");
			rs.getString("L_NAME");
			String concat=rs.getString("S_ID")+"-"+rs.getString("F_NAME")+" "+rs.getString("L_NAME");
			list=concat;
	...
	public  ArrayList<String> fetchallsubject() throws ClassNotFoundException, SQLException {
		dbc = new DBconnection();
		Connection c = null;
		c = dbc.createConnection();
		...
		PreparedStatement ps = c.prepareStatement("SELECT SUBJECT_ID, SUBJECT_NAME from SUBJECT");
		ResultSet rs =  (ResultSet)ps.executeQuery();
		while (rs.next()) 
		{
			String list;
			...
			//rs.getString("L_NAME");
			String concat=rs.getString("SUBJECT_ID")+"-"+rs.getString("SUBJECT_NAME");
			list=concat;
	...
	public  ArrayList<String> fetchallFaculty() throws ClassNotFoundException, SQLException {
		dbc = new DBconnection();
		Connection c = null;
		c = dbc.createConnection();
		
		ArrayList<String> u1 = new ArrayList<String>();
		PreparedStatement ps = c.prepareStatement("select FACULTY_ID, FACULTY_NAME from FACULTY");  
		ResultSet rs =  (ResultSet)ps.executeQuery();
		while (rs.next()) 
		{
				String list =rs.getString("FACULTY_ID")+"-"+rs.getString("FACULTY_NAME");
	...
	public static int Save(AddFacultyModel addFacultyModel) {
		...
		Connection	c=(Connection) dbc.createConnection();
			...
			PreparedStatement  ps =c.prepareStatement("insert into FACULTY(FACULTY_ID,FACULTY_NAME,EMAIL) values(?,?,?)");
			...
			PreparedStatement pstmt = c.prepareStatement("insert into CREDENTIALS(PASSWORD,U_ID,ROLE) values(?,?,?);");
	...
	public  int Save(AddStudentModel sm)
	{
		...
		try 
		{
			...
			PreparedStatement ps=c.prepareStatement("insert into STUDENT(S_ID,ADDRESS,PHOTO,F_NAME,L_NAME,EMAIL,PHONE,CURRENT_SEMESTER,BirthPlace) values(?,?,?,?,?,?,?,?,?)");  
	...
	public static void addSubject(ArrayList<SubjectModel> array) throws Exception
	{	
		DBconnection db=new DBconnection();
		Connection con=db.createConnection();
	...
	{	
		...
		try 
		{
			...
			pstmt =c.prepareStatement("update SUBJECT SET SUBJECT_NAME = ?"+","+"SYLLABUS = ?"+",CREDIT=?"+",SPECIALIZATION = ?"+",SEMESTER = ?"+" where FACULTY_ID='" + updateSubject.getSubject_Id() + "';");  
			//System.out.println(getFaculty.getFaculty_Id());
	...
	{	
		...
		try 
		{
			...
			pstmt =c.prepareStatement("DELETE FROM SUBJECT WHERE SUBJECT_ID='" + deleteSubject.getSubject_Id() + "';");  
			//System.out.println(getFaculty.getFaculty_Id());
			pstmt.executeUpdate();
	...
	public static void updateGrades(User u)
	{
		...
		try
		{
			...
			{
			PreparedStatement ps1=con.prepareStatement("select SUBJECT_ID from SUBJECT where SUBJECT_NAME =?");
			ps1.setString(1,u.getSubject().get(i));
			ResultSet rs= ps1.executeQuery();
			rs.next();
			System.out.println("subject id "+rs.getString(1));
			PreparedStatement ps=con.prepareStatement("update STU_SUB set marks=?, grade=?, result=? where SUBJECT_ID = ? and S_ID=?");
	...
	public static void searchSubject(User u)
	{
		...
		try
		{
			...
			DBconnection db=new DBconnection();
			Connection con=db.createConnection();
			PreparedStatement ps1=con.prepareStatement("select CURRENT_SEMESTER from STUDENT WHERE S_ID=?");
			ps1.setString(1, u.getS_id());
			ResultSet rs1= ps1.executeQuery();
			rs1.next();
			String sem=rs1.getString(1);
			PreparedStatement ps=con.prepareStatement("select SUBJECT_NAME from SUBJECT where SEMESTER=? and SUBJECT_ID in(select SUBJECT_ID from STU_SUB WHERE S_ID=?)");
			ps.setString(1, sem);
	...
	try{
		...
		String query="Select SUBJECT_NAME from SUBJECT where SEMESTER=? and SUBJECT_ID in (select SUBJECT_ID from STU_SUB where S_ID='"+sid+"')";
	...
	public void getFinancial(StudentModel student,FinancialModel usr) throws FileNotFoundException{
		try {
			DBconnection db=new DBconnection ();	
			Connection con=db.createConnection();	
			...
			String sql;
			//sql = "select SEMESTER,TOTAL_FEE,FEE_PAID,PAY_DATE from ooad_db.STU_FEE where S_ID =' " +output.getS_id()+"';";
	...
	static String image,role,name1;
	public void checkdb(UserModel retrieve) throws FileNotFoundException{
		...
		try {
			...
			String sql;
			sql="select ROLE from CREDENTIALS where U_ID= '" + retrieve.getUserid() +"' and PASSWORD='" + retrieve.getPwd()+"'";
			PreparedStatement stmt = po.prepareStatement(sql);
			//stmt.setString(1,retrieve.getUserid());
			//stmt.setString(2,retrieve.getPwd());
			ResultSet rs = stmt.executeQuery();
			PreparedStatement stmt1 = po.prepareStatement("select PHOTO,F_NAME from STUDENT where S_ID=?");
			stmt1.setString(1, retrieve.getUserid());
			ResultSet rs1 = stmt1.executeQuery();
			while(rs1.next()){
				retrieve.setDestpath(rs1.getString(1));
				retrieve.setName(rs1.getString(2));
	...
	public  ArrayList<GradeModel>  getdetails(String sid,String sem)
	{
		...
		try {
			int i=0;
			String sql;
			sql = "SELECT SUBJECT.SUBJECT_ID,SUBJECT_NAME,GRADE,RESULT FROM STU_SUB,SUBJECT  where STU_SUB.SUBJECT_ID=SUBJECT.SUBJECT_ID AND S_ID= '" + sid +"' and SEMESTER='" + sem+"'";
			ps=(PreparedStatement) con.prepareStatement(sql);
		...
		return name; 
			...
			public  ArrayList<LibraryModel> searchBook (String sid)
				...
				try
				{
					...
					DBconnection db=new DBconnection();System.out.println("connection");
					Connection con= db.createConnection();
					PreparedStatement ps= (PreparedStatement) con.prepareStatement("SELECT * FROM LIBRARY where S_ID=?");
					ps.setString(1, sid);
					System.out.println("ps is"+ps);
					System.out.println("Issued Books");
					ResultSet rs= ps.executeQuery();System.out.println("rs is\n"+rs);
					while(rs.next())
					{
					    ...
					    System.out.println("in search sub"+lm.getBook_name());// Date Issue_date;
	...
	i++;
			...
			DBconnection db=new DBconnection ();	
				Connection con=db.createConnection();
				String query1="INSERT INTO REPORT_IN(S_ID,INTEREST) VALUES(?,?)";
				String query = "UPDATE STUDENT SET F_NAME= ?,L_NAME= ?,DOB= ?,ADDRESS= ?,EMAIL= ?,PHONE= ? ,INTEREST=? where S_ID=? ";
			try
			{
				
			System.out.println("sid new"+studentinfo.getSid()+"name"+studentinfo.getFname()+"dob"+studentinfo.getDob());
		...
		ps.setString(8, studentinfo.getSid());
			...
			{	
				...
				String s1=studentinfo.getIntereststudent().get(i);
				ps1.setString(1, studentinfo.getSid());
				ps1.setString(2, s1);
				System.out.println("intesr"+s1);
		...
		//	System.out.println("interest in db"+interest);
			...
			public int insertStudentSubject(String subj, String tok1) {
				...
				Connection	c=(Connection) dbc.createConnection();
					...
					PreparedStatement ps = c.prepareStatement("select CURRENT_SEMESTER FROM STUDENT where S_ID =?");
					PreparedStatement ps1 = c.prepareStatement("select SEMESTER FROM SUBJECT where SUBJECT_ID =?");
					ps.setString(1,tok1);
	...
	public ArrayList<StudentModel> fetchStudentSubject(StudentModel studentModel, String userid) {
		dbc =new DBconnection();
		...
		Connection	c=(Connection) dbc.createConnection();
			...
			PreparedStatement  ps =c.prepareStatement("select SUBJECT_NAME from SUBJECT where SUBJECT_ID in(select SUBJECT_ID from STU_SUB where S_ID = ?);");
	...
	public int insertFacultySubject(String sub, String tok) {
		...
		Connection	c=(Connection) dbc.createConnection();
			...
			PreparedStatement  ps =c.prepareStatement("insert into FACULTY_SUB(FACULTY_ID,SUBJECT_ID) values(?,?)");
			ps.setString(1, tok);
	...
	public ArrayList<FacultyModel> fetchFacultySubject(FacultyModel facultyModel)
	{
		...
		Connection	c=(Connection) dbc.createConnection();
			...
			PreparedStatement  ps =c.prepareStatement("select SUBJECT_NAME from SUBJECT where SUBJECT_ID in(select SUBJECT_ID from FACULTY_SUB where FACULTY_ID = '"+ facultyModel.getFaculty_Id()+"');");
	...
	public static void InsertLibrary(LibraryModel lm)
	{
		try
		{
			...
			PreparedStatement ps1=con.prepareStatement("insert into LIBRARY values (?,?,?,?,?,?)");
	...
	public int checkNoOfsubject(String tok1) 
	{
		...
		{
		Connection	c=(Connection) dbc.createConnection();
			PreparedStatement ps = c.prepareStatement("select SUBJECT_ID from STU_SUB where S_ID = ? and SUBJECT_ID in( select SUBJECT_ID from SUBJECT where SEMESTER = (select CURRENT_SEMESTER from STUDENT where S_ID = ?));");
			ps.setString(1,tok1);
			ps.setString(2,tok1);
			int count=0;
			ResultSet rs = ps.executeQuery();
...
try
{
	...
	System.out.println("connection");
	Connection con= db.createConnection();
	PreparedStatement ps= (PreparedStatement) con.prepareStatement("SELECT * FROM INTEREST");
	ResultSet rs= ps.executeQuery();
	while(rs.next())
	{
...
public void insertfrnd(FriendModel f){
	...
	try {
		...
		sql = "insert into SIS.FRIEND (S_ID,FRIEND_NAME,FRIEND_ID,status,requst) values(?,?,?,?,?)";
		PreparedStatement prep1 =con.prepareStatement(sql);
...
public void insertNfrnd(FriendModel f){
	...
	try {
		...
		System.out.println("sid"+Sid);
		System.out.println(fm.getFrndtry_name()+fm.getSrch_frnd());
		sql = "insert into SIS.FRIEND (requst) values(?) where S_ID=? ";
		PreparedStatement prep1 =con.prepareStatement(sql);
		//prep1.setString(1, id);
		prep1.setString(3,Sid );
		//prep1.setString(2, Sid);
		prep1.setInt(1, 1);
		//prep1.setInt(5, 0);
		prep1.execute();
		System.out.println(id+frndname);
...
ArrayList<FriendModel> token=new ArrayList<FriendModel>(); 

 String[] POLL;
public void tokenIntrst(){
		...
		FriendModel fm=new FriendModel();
			...
			try {
				int i=0;
				 String abc;
				 String s;
			s="SELECT S_ID,INTEREST from STUDENT ";
					PreparedStatement prep1 =con.prepareStatement(s);
					String query="INSERT INTO REPORT_IN(S_ID,INTEREST) VALUES(?,?)";
					PreparedStatement prep =con.prepareStatement(query);
					
				ResultSet rs1 = prep1.executeQuery();
				
				int limit=std.getIntereststudent().size();	
				while(rs1.next()){
					
					FriendModel fm1=new FriendModel();
					fm1.setS_ID(rs1.getString("S_ID"));
					fm1.setINTEREST(rs1.getString("INTEREST"));
					System.out.println("token int:"+fm1.getINTEREST());
					
					token.add(i,fm1);
					for(int m=0;m<limit;m++)
					{	
						String s1=std.getIntereststudent().get(m)+" ,";
						prep.setString(1, fm1.getS_ID());
						prep.setString(2, s1);
...
public  ArrayList<FriendModel>  report(String intrst){
		...
		try {
			int i=0;
			String sql;
			sql = "SELECT STUDENT.S_ID,F_NAME,PHOTO,CURRENT_SEMESTER from REPORT_IN,STUDENT where STUDENT.S_ID= REPORT_IN.S_ID and REPORT_IN.INTEREST=?";
			
			
			PreparedStatement prep1 =con.prepareStatement(sql);
           
			prep1.setString(1, intrst);
			...
			while(rs.next()){
				...
				fm.setS_ID(rs.getString("S_ID"));
				fm.setDestpath(rs.getString("PHOTO"));
				fm.setFrnd_Address(rs.getString("CURRENT_SEMESTER"));
				System.out.println("fname"+rs.getString("F_NAME"));
				 repo.add(i,fm);
				 System.out.println(fm.getFrndtry_name());
					System.out.println(fm.getFrnd_Address());
...
ArrayList<FriendModel> bith=new ArrayList<FriendModel>(); 	
public  ArrayList<FriendModel>  birth(String intrst){
	...
	try {
		int i=0;
		String sql;
		sql = "SELECT STUDENT.S_ID,F_NAME,CURRENT_SEMESTER from STUDENT where BirthPlace=?";
...
public void addrequst(String intsrt){
DBconnection db=new DBconnection ();
	...
	try {
		int i=0;
		String sql;
		sql = "INSERT INTO FRIEND(S_ID,FRIEND_ID,status) VALUES(?,?,?)";
		
		
		PreparedStatement prep1 =con.prepareStatement(sql);
       System.out.println("sid="+Sid);
		prep1.setString(1, Sid);
		prep1.setString(2, intsrt);
		prep1.setInt(3, 1);
